...
#define SPT_CHECK_NO_ENTRY_MSG(...)		SPT_CHECK_MSG(false, __VA_ARGS__)
#define SPT_CHECK_NO_ENTRY()			SPT_CHECK(false)

#else

#define SPT_CHECK_MSG(Expression, ...)
#define SPT_CHECK(Expression)

#define SPT_CHECK_NO_ENTRY_MSG(...)
#define SPT_CHECK_NO_ENTRY()

#endif // DO_CHECKS

#define SPT_STATIC_CHECK_MSG(Expression, ...)	static_assert(Expression, __VA_ARGS__)
