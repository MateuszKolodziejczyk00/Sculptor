...
SPT_REGISTER_SCENE_RENDER_SYSTEM(ShadowMapsRenderSystem)
...
ShadowMapsRenderSystem::~ShadowMapsRenderSystem()
{
	for (RenderView* shadowMapRenderView : m_shadowMapsRenderViews)
	{
		rsc::DestroyRenderView(shadowMapRenderView);
	}
}
...
void ShadowMapsRenderSystem::Update(const SceneUpdateContext& context)
{
	...
	AssignShadowMaps(context.mainRenderView);
...
lib::DynamicArray<RenderView*> ShadowMapsRenderSystem::GetShadowMapViewsToUpdate() const
{
	...
	for (RenderSceneEntity light : lightsToUpdate)
	{
		...
		{
			renderViewsToUpdate.emplace_back(m_shadowMapsRenderViews[static_cast<SizeType>(pointLightShadowMap.shadowMapFirstFaceIdx) + faceIdx]);
		}
	}
...
void ShadowMapsRenderSystem::CreateShadowMapsRenderViews()
{
	...
	{
		RenderView*& renderView = m_shadowMapsRenderViews.emplace_back();
		RenderViewDefinition renderViewDef;
		renderViewDef.renderStages = ERenderStage::ShadowMapRendererStages;
		renderView = rsc::CreateRenderView(renderViewDef);
	}
}

void ShadowMapsRenderSystem::AssignShadowMaps(const RenderView& mainView)
{
	SPT_PROFILER_FUNCTION();
...
void ShadowMapsRenderSystem::UpdateShadowMapRenderViews(RenderSceneEntity owningLight, const PointLightData& pointLight, Uint32 shadowMapBeginIdx)
{
	...
	for (SizeType faceIdx = 0; faceIdx < faceRotations.size(); ++faceIdx)
	{
		...
		RenderView* renderView = m_shadowMapsRenderViews[renderViewIdx];
		...
		renderView->SetOutputRes(shadowMapResolution);
