...
MultiScatteringResult ComputeMultiScatteringValues(in float3 location, in float3 sunDirection)
{
    ...
    const Sphere atmophereSphere = { ZERO_VECTOR, u_atmosphereParams.atmosphereRadiusMM };
    ...
    for (int i = 0; i < sqrtDirectionsNum; ++i)
    {
        for (int j = 0; j < sqrtDirectionsNum; ++j)
        {
            ...
            const Ray ray = { location, rayDirection };

            const float atmopshereDist = RaySphereIntersect(ray, atmophereSphere);
            const float groundDist = RaySphereIntersect(ray, groundSphere);
