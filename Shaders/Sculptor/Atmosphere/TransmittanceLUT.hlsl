...
{
    const Ray rayToSun = { location, sunDirection };
    const Sphere groundSphere = { ZERO_VECTOR, u_atmosphereParams.groundRadiusMM };

    if(RaySphereIntersect(rayToSun, groundSphere) > 0.f)
    {
        return 0.f;
    }
    ...
    const float rayDistance = RaySphereIntersect(rayToSun, atmosphereSphere);
    ...
    {
        const float newT = (i + 0.3f) * rcpStepsNum * rayDistance;
