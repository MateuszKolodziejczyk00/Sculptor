...
void GenerateGeometryNormalsCS(CS_INPUT input)
{
    ...
    if (all(pixel < outputRes))
    {
        ...
        const float3 sampleVL = NDCToViewSpace(float3(uv * 2.f - 1.f, sampleDepth), u_sceneView);

        const float2 downUV = uv + float2(0.f, -1.f * pixelSize.y);
        const float3 downSampleVL = NDCToViewSpace(float3(downUV * 2.f - 1.f, verticalDepth.x), u_sceneView);

        const float2 upUV = uv + float2(0.f, 1.f * pixelSize.y);
        const float3 upSampleVL = NDCToViewSpace(float3(upUV * 2.f - 1.f, verticalDepth.y), u_sceneView);

        const float2 leftUV = uv + float2(-1.f * pixelSize.x, 0.f);
        const float3 leftSampleVL = NDCToViewSpace(float3(leftUV * 2.f - 1.f, horizontalDepth.x), u_sceneView);

        const float2 rightUV = uv + float2(1.f * pixelSize.x, 0.f);
        const float3 rightSampleVL = NDCToViewSpace(float3(rightUV * 2.f - 1.f, horizontalDepth.y), u_sceneView);
