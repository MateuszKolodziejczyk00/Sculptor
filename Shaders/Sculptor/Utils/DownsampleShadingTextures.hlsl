...
void DownsampleShadingTexturesCS(CS_INPUT input)
{
	...
	if(pixel.x < outputRes.x && pixel.y < outputRes.y)
	{
		...
		const float roughness0 = u_roughnessTexture.Load(inputPixel0);
		const float roughness1 = u_roughnessTexture.Load(inputPixel1);
		const float roughness2 = u_roughnessTexture.Load(inputPixel2);
		const float roughness3 = u_roughnessTexture.Load(inputPixel3);

		const float3 normal0 = DecodeGBufferNormal(u_tangentFrame.Load(inputPixel0));
		const float3 normal1 = DecodeGBufferNormal(u_tangentFrame.Load(inputPixel1));
		const float3 normal2 = DecodeGBufferNormal(u_tangentFrame.Load(inputPixel2));
		const float3 normal3 = DecodeGBufferNormal(u_tangentFrame.Load(inputPixel3));
		...
		const float roughness = (roughness0 * weight0 + roughness1 * weight1 + roughness2 * weight2 + roughness3 * weight3) * rcpWeightSum;
