...
[[shader_params(GrassBladesVisibilityPassConstants, PARAMS_GRASS_BLADES_VISIBILITY_PASS)]]
...
{
	const uint bladesNum      = PARAMS_GRASS_BLADES_VISIBILITY_PASS->bladesNumLODs[GRASS_BLADES_LOD].Load(0u);
	...
	for (uint vertexIdx = input.localID; vertexIdx < verticesNum; vertexIdx += GRASS_VISIBILITY_MS_GROUP_SIZE)
	{
		...
		const GrassBladeDef bladeDef   = PARAMS_GRASS_BLADES_VISIBILITY_PASS->bladeDefsLODs[GRASS_BLADES_LOD].Load(bladeIdx);
		...
		outVertices[vertexIdx].clipSpace = mul(VIEW->sceneView.viewProjectionMatrix, float4(vertexLocationWS, 1.f));
	}
