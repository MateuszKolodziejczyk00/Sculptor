...
groupshared bool gs_isValid[SHARED_SAMPLES_RES][SHARED_SAMPLES_RES];
	...
	if(all(pixel < u_constants.resolution))
	{
		...
		for(int y = -1; y <= 1; ++y)
		{
			...
			for(int x = -1; x <= 1; ++x)
			{
				...
				{
					const bool isValidSample = gs_isValid[input.localID.x + x + BORDER_SIZE][input.localID.y + y + BORDER_SIZE];
					if (!isValidSample)
					{
						continue;
					}
