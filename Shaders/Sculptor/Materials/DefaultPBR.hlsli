...
MaterialEvaluationOutput EvaluateMaterial(MaterialEvaluationParameters evalParams, SPT_MATERIAL_DATA_TYPE materialData)
{
    ...
    if(materialData.baseColorTextureIdx != IDX_NONE_32)
    {
        baseColor = u_materialsTextures[NonUniformResourceIndex(materialData.baseColorTextureIdx)].SPT_MATERIAL_SAMPLE(u_materialTexturesSampler, evalParams.uv).rgb;
    ...
    {
        const float2 metallicRoughness = u_materialsTextures[NonUniformResourceIndex(materialData.metallicRoughnessTextureIdx)].SPT_MATERIAL_SAMPLE(u_materialTexturesSampler, evalParams.uv).xy;
    ...
    {
        float3 textureNormal = u_materialsTextures[NonUniformResourceIndex(materialData.normalsTextureIdx)].SPT_MATERIAL_SAMPLE(u_materialTexturesSampler, evalParams.uv).xyz;
    ...
    {
        emissiveColor *= u_materialsTextures[NonUniformResourceIndex(materialData.emissiveTextureIdx)].SPT_MATERIAL_SAMPLE(u_materialTexturesSampler, evalParams.uv).rgb;
    }
